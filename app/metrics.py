...
def get_graphic_product_category_metrics():
    ...
    return {category.name: Products.objects.filter(category=category).count() for category in categories}


def get_graphic_product_brand_metric():
    brands = Brand.objects.all()
    return {brand.name: Products.objects.filter(brand=brand).count() for brand in brands}
